...
bash scripts/eval_bnmt.sh $src $tgt $data $save/checkpoint_best.pt $ptm > $save/gen_best_${src}${tgt}.txt
bash scripts/avg10.sh $save/ 10
bash scripts/eval_bnmt.sh $src $tgt $data $save/avg10.pt $ptm > $save/gen_avg10_${src}${tgt}.txt
