...
from .der_loss import domain_entity_routing_loss,make_fake_entity
...
def top2gating(
    ...
    if (entity_ids is not None) and (len(entity_ids) > 0):
        ...
        der_loss = domain_entity_routing_loss(logits=logits,
                                              gate_score=gates,
                                              entity_index=entity_ids,
                                              entity_length=entity_len,
                                              tau=1,
                                              epsilon=0.1)
        # print("der loss:", der_loss)
    ...
    if batch_prioritized_routing: # false
    ...
    # Compute l_aux     #(loss_all = alpha * loss_ce + beta * loss_aux  , alpha=1 beta=0.01 )
    me = torch.mean(gates, dim=0) # 平均概率
    ce = torch.mean(mask1.to(gates.dtype), dim=0) # 词数占比
    l_aux = torch.mean(me * ce)
    l_aux = l_aux * num_experts * num_experts

    l_aux += der_loss # 领域路由损失
...
class Top2Gate(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor, mask: Optional[torch.Tensor] = None, moe_eval_capacity_length: Optional[int] = None, prefix_tokens: Optional[torch.Tensor] = None,
                # domain entity routing loss
                entity_ids=None,
                entity_len = None ,
