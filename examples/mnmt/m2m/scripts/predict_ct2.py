...
def main(args):
    ...
    # translator = ctranslate2.Translator(args.model_path, compute_type="int8", device="cuda")
    translator = ctranslate2.Translator(args.model_path, device="cuda")
    
    # tokenizer = transformers.AutoTokenizer.from_pretrained(args.model_path)
    # tokenizer = transformers.AutoTokenizer.from_pretrained("/mnt/f/down/m2m100_418M/")
    ...
    def translate_batch(text_ls, tokenizer ,translator, src_lang, tgt_lang, args):
        tokenizer.src_lang = src_lang
        source_ls = [tokenizer.convert_ids_to_tokens(tokenizer.encode(text)) for text in text_ls]
        target_prefix = [tokenizer.lang_code_to_token[tgt_lang]]
        results = translator.translate_batch(source_ls, target_prefix=[target_prefix]* len(source_ls), max_batch_size=args.batch_size,  beam_size=args.beam_size)
        # print("results",results)
        # res = []
        # for result in results:
        #     tgt_tokens = result.hypotheses[0][1:]
        #     print("res:", type(tgt_tokens) ,tgt_tokens)
        #     print("ids", tokenizer.convert_tokens_to_ids(tgt_tokens))
        #     text = tokenizer.decode(tokenizer.convert_tokens_to_ids(tgt_tokens))
        #     print("text", text)
        #     res.append(text)
        target_ls = [result.hypotheses[0][1:] for result in results]
        target_ls = [tokenizer.decode(tokenizer.convert_tokens_to_ids(target)) for target in target_ls]
    ...
    for lang_dir, df_lang in df.groupby('lang_dir'):
        ...
        predicted_texts = translate_batch(src_texts, tokenizer ,translator,src_lang=src_lang, tgt_lang=tgt_lang, args=args )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-beam","--beam_size", type=int, default=5, help="beam size")
