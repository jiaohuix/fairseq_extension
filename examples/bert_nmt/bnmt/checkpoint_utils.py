...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss):
    ...
    if len(checkpoints) > 0 and trainer.should_save_checkpoint_on_current_rank:
        ...
        for cp in checkpoints[1:]:
            if cfg.write_checkpoints_asynchronously:
                ...
                # 把loras/checkpoint_best拷贝到loras/checkpoint.best_xx
                src_lora = get_lora_dir(checkpoints[0])
                dest_lora = get_lora_dir(cp)
                shutil.rmtree(dest_lora)
                shutil.copytree(src_lora, dest_lora)
    ...
    if (
        ...
        for old_chk in checkpoints[cfg.keep_interval_updates :]:
            old_chk_lora = get_lora_dir(old_chk)
            if os.path.lexists(old_chk):
                os.remove(old_chk)
                shutil.rmtree(old_chk_lora) # 删除旧的lora
