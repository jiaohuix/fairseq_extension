...
from bert_nmt_extensions.modules import AveragePooler,ConvPooler,RandPooler,MaxPooler
from bert_nmt_extensions.modules import build_dynamic_gate
from bert_nmt_extensions.modules import create_select_layer
from fairseq.modules import GradMultiply


class BertFusedEncoderLayer(TransformerEncoderLayerBase):
    def __init__(self, cfg,  bert_gate=True, bertattn=None):
        super().__init__(cfg)
        ...
        if cfg.use_pooler:
            self.bert_attn = None
        elif bertattn is not None and isinstance(bertattn,(MultiheadAttention)): # 共享bertattn
            self.bert_attn = bertattn
        elif self.bert_gate: # 独享
            self.bert_attn = self.build_bert_attention(self.embed_dim, cfg)
        else:  # 不构建
            self.bert_attn = None
        ...
        self.type = getattr(cfg, 'gate_type', "dynamic")
        self.gate = build_dynamic_gate(self.embed_dim,cfg.gate_type) if self.dynamic_gate else  None

        ### layer select for bert ###
        self.select_layer = create_select_layer(ls_type=cfg.ls_type, bert_output_layer=cfg.bert_output_layer) if self.bert_gate else None

    @staticmethod
    def build_bert_attention(embed_dim, cfg):
        return MultiheadAttention(
            ...
            # q_noise=cls.quant_noise,
            # qn_block_size=cls.quant_noise_block_size,
            # xformers_att_config=cfg.encoder.xformers_att_config,
        )

    # TODO: Fix hardcoding of bert dimension
    def build_bert_pooler(self, embed_dim,cfg, bert_dim=768):
        pooler_map = {"avg":AveragePooler, "cnn":ConvPooler, "rand":RandPooler,"max":MaxPooler}
        pooler = pooler_map.get(cfg.pooler_type,AveragePooler)(in_features = bert_dim, out_features = embed_dim)
        return pooler
    ...
    def forward_fusion(self,x,x1,bert_encoder_outs,bert_encoder_padding_mask):

        bert_encoder_out = self.select_layer(bert_encoder_outs)
    ...
    def forward(
        ...
        if self.bert_gate:
            ...
            if self.cfg.fuse_lr_multiply > 1:
                # scale gradients (this only affects backward, not forward)
                x = GradMultiply.apply(x, 1 / self.cfg.fuse_lr_multiply)

            x = self.forward_fusion(x, x1, bert_encoder_outs, bert_encoder_padding_mask)

            if self.cfg.fuse_lr_multiply > 1:
                # scale gradients (this only affects backward, not forward)
                x = GradMultiply.apply(x, self.cfg.fuse_lr_multiply)
...
class BertFusedDecoderLayer(TransformerDecoderLayerBase):
    def __init__(
        ...
        bert_gate = True,bertattn=None # <--bert fused
    ):
        super().__init__(
            cfg,
        ...
        if not no_encoder_attn: # ?
            ...
            if cfg.use_pooler:
                self.bert_attn = None
            elif bertattn is not None and isinstance(bertattn, (MultiheadAttention)):
                self.bert_attn = bertattn # 共享参数
            elif self.bert_gate:
                self.bert_attn = self.build_bert_attention(self.embed_dim, cfg)
            else:
                self.bert_attn = None
    ...
    def build_bert_attention(embed_dim, cfg):
        return MultiheadAttention(
            ...
            # self_attention=True, #  key = query ; value = query
            kdim=cfg.bert_out_dim,  # kdim -> embed_dim
            vdim=cfg.bert_out_dim,
            # q_noise=cls.quant_noise,
            # qn_block_size=cls.quant_noise_block_size,
            # xformers_att_config=cfg.encoder.xformers_att_config,
    ...
    def forward_fusion(self,x,x1, bert_encoder_outs,bert_encoder_padding_mask,incremental_state):
        bert_encoder_out = self.select_layer(bert_encoder_outs)
        if self.bert_attn is not None:
            x2, _ = self.bert_attn(
                query=x,
        ...
        if self.encoder_attn is not None and encoder_out is not None:
            ...
            if self.bert_gate:
                ...
                if self.cfg.fuse_lr_multiply > 1:
                    # scale gradients (this only affects backward, not forward)
                    x = GradMultiply.apply(x,  1 / self.cfg.fuse_lr_multiply)
                x = self.forward_fusion(x,x1, bert_encoder_outs,bert_encoder_padding_mask,incremental_state)
                if self.cfg.fuse_lr_multiply > 1:
                    # scale gradients (this only affects backward, not forward)
                    x = GradMultiply.apply(x,  self.cfg.fuse_lr_multiply)
