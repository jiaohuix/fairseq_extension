...
reverse_outdir = os.path.join(out_root, data_name.split("/")[-1], reversed_lang_pair)
...
# 输出codes文件
tmp_corpus_file=f"./corpus_{lang_pair}.tmp"
codes_file = os.path.join(outdir, "codes.txt")
codes_rev_file = os.path.join(reverse_outdir, "codes.txt")
if os.path.exists(codes_rev_file): # todo： 判断行数
    print(f"already exists {codes_rev_file} ---> {codes_file}")
    shutil.copy(codes_rev_file, codes_file)
    codes_file = codes_rev_file
else:
    print(f"train bpe: {codes_file}")
    # 1 获取文本并保存
    corpus = concatenate_datasets([dataset[split] for split in splits])
    text_ls = corpus[src_lang] + corpus[tgt_lang]
    text_ls = [t.strip()+"\n" for t in text_ls]
    write_file(text_ls, tmp_corpus_file)
