...
Modify:
    task.finetune_bert:  这个难道不是加在model里面吗？
    task.warmup_from_nmt ？
    checkpoint_utils
    trainner
"""
import argparse
...
from bert_nmt_extensions import checkpoint_utils
from bert_nmt_extensions.trainer import Trainer
...
def main(cfg: FairseqConfig) -> None:
    ...
    logger.info(f"cfg.taskcfg.taskcfg.taskcfg.taskcfg.taskcfg.taskcfg.taskcfg.taskcfg.taskcfg.taskcfg.taskcfg.taskcfg.taskcfg.task: {cfg.task}")
    ...
    ####################################### BERT-FUSED ##############################################
    if not cfg.task.finetune_bert and not cfg.model.use_lora:
        for param in model.bert_encoder.parameters():  # 你修改了我的lora！！！
    ...
    if cfg.task.freeze_nmt: # 没考虑到bert
        for name, param  in model.named_parameters():
            if "gate" in name:
                print(f"defreeze {name}")
    ...
    cfg.checkpoint.warmup_from_nmt = cfg.task.warmup_from_nmt
    cfg.checkpoint.warmup_nmt_file = cfg.task.warmup_nmt_file
    ...
    # if cfg.task.warmup_from_nmt:
    #     checkpoint_utils.save_checkpoint(cfg.checkpoint, trainer, epoch_itr, valid_losses[0],warmup_from_nmt=True)
...
def bert_nmt_parser(parser):
    ...
    group.add_argument('--finetune-bert',action='store_true',)
    group.add_argument('--freeze-nmt',action='store_true',)
    group.add_argument('--warmup-from-nmt', action='store_true', )
