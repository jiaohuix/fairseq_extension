...
warmup_from_nmt
4/17
1.添加remove_module_params，保存时删除bert的参数
2.添加lora的权重保存
"""
import os
import sys
...
from bert_nmt_extensions.checkpoint_utils import get_lora_params,load_lora_params,remove_module_params
...
class Trainer(object):
    ...
    def state_dict(self):
        state_dict = {
            ...
            # remove bert params
            "model": remove_module_params(self.model),
            # lora config and params
            "lora": get_lora_params(self.model),
    ...
    def load_checkpoint(
        ...
        reset_meters=False,
        warmup_from_nmt=False,  # <--- bert fused 如果True，模型加载参数strict=False,可以多或少加载参数
    ):
        """
        ...
        if bexists:
            ...
            try:
                ...
                if (
                    ...
                    logger.info(self.model)
                strict = False if warmup_from_nmt else True
                if self.model.use_lora:
                    strict = False
                self.model.load_state_dict(
                    state["model"], strict=strict, # <--- bert fused
                    # state["model"], strict=False if warmup_from_nmt else True, # <--- bert fused
                ...
                #### Load LoRA ###
                if self.model.use_lora and safe_hasattr(state,"lora"):
                    load_lora_params(self.model, lora_params=getattr(state,"lora", None))
                    if safe_hasattr(state,"lora"):
                        del state["lora"]
