'''
both sentence level and entity level.
'''
...
@register_criterion("label_smoothed_ce_with_multi_granularity_contrastive")
class LabelSmoothedCrossEntropyCriterionWithEntityContrastive(
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if sample.get("entity_info", None) is not None:
            entity_ct_loss, entity_num = self.get_entity_contrastive_loss(
                encoder_out,
                reversed_encoder_out,
                sample,
                reverse_sample,
            )
            setence_ct_loss = self.get_contrastive_loss(
                        encoder_out,
                        reversed_encoder_out,
                        sample,
                        reverse_sample,)
            ...
            all_loss= loss \
                      + entity_ct_loss * self.contrastive_lambda * ntokens / entity_num \
                      + setence_ct_loss * self.contrastive_lambda * ntokens / nsentences
        else:
            all_loss = loss
        logging_output = {
            "loss": loss.data,
        ...
        if isinstance(entity_ct_loss, int):
            logging_output["entity_ct_loss"] = 0
        else:
            logging_output["entity_ct_loss"] = utils.item(entity_ct_loss.data)

        if isinstance(entity_ct_loss, int):
            logging_output["setence_ct_loss"] = 0
        else:
            logging_output["setence_ct_loss"] = utils.item(setence_ct_loss.data)
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        setence_ct_loss = utils.item(
            sum(log.get("setence_ct_loss", 0) for log in logging_outputs)
        )
        metrics.log_scalar(
            "setence_ct_loss",
            setence_ct_loss / nsentences / math.log(2),
            nentities,
            round=3,
        )
