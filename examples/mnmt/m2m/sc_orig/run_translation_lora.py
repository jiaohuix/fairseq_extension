...
"""
数据格式：
{ "translation": { "en": "Others have dismissed him as a joke.", "ro": "Alții l-au numit o glumă." }, lang:'en-ro'}
{ "translation": { "en": "And some are holding out for an implosion.", "ro": "Iar alții așteaptă implozia." }, lang:'en-ro' }
...
class ModelArguments:
    ...
    lora_training: Optional[bool] = field(
        default=False,
    ...
    use_dora: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to use dora training."}
    )


    modules_to_save: Optional[str] = field(
        default=None,
        metadata={"help": "List of modules apart from adapter layers to be set as trainable and saved in the final checkpoint. (such as embed_tokens)"}
    )
...
def main():
    ...
    if model_args.lora_training:
        ...
        # peft_config = LoraConfig(inference_mode=False, r=model_args.lora_rank,
                                        #  lora_alpha=model_args.lora_alpha, lora_dropout=model_args.lora_dropout,
                                        #  target_modules=target_modules)
        print(model_args.lora_target.split(","))                                  
        peft_config = LoraConfig(inference_mode=False, 
                                r=model_args.lora_rank,
                                lora_alpha=model_args.lora_alpha, 
                                lora_dropout=model_args.lora_dropout,
                                target_modules=model_args.lora_target.split(","),
    ...
    train_df = pd.read_json(data_args.train_file, lines=True)
    valid_df = pd.read_json(data_args.validation_file, lines=True)

    total_size = len(train_df)
    temp = 5


    lang_counts = dict(train_df['lang'].value_counts())
    # 创建一个新的字典，将原字典的键和值互换
    reverse_lang = lambda x: x
    reversed_lang_counts = {key.split("-")[1]+"-"+key.split("-")[0] : value for key, value in lang_counts.items()}
    # 合并原字典和新字典
    dual_lang_counts = {**lang_counts, **reversed_lang_counts}
    dual_lang_prob = {key: (value/ total_size) ** (1.0 / temp) for key, value in dual_lang_counts.items()}
    ...
    for ll in lang_list:
        ...
        # train_datasets = load_dataset('json', data_files=[data_args.train_file])
        train_datasets = train_datasets.filter(lambda example: example["translation"][source_lang] is not None  and   example["translation"][target_lang] is not None).map(lambda ex:ex, remove_columns=["lang"])
        # valid_datasets = load_dataset('json', data_files=[data_args.validation_file])
        valid_datasets = valid_datasets.filter(lambda example: filter_lang_pair(example)).map(lambda ex:ex, remove_columns=["lang"])  # todo: zh-en en-zh会被变成2份

        # sample strategy
        # train_datasets = train_datasets.filter(lambda example: random.random()<=train_ratio)
        # valid_datasets = valid_datasets.filter(lambda example: random.random()<=valid_ratio)
        # temperature sampling
        lang = f"{source_lang}-{target_lang}"
        sample_p = dual_lang_prob[lang]
        # train_datasets = train_datasets.filter(lambda example: random.random()<=sample_p)
        # valid_datasets = valid_datasets.filter(lambda example: random.random()<=sample_p)
    ...
    print(f"train_dataset size {len(train_dataset_merge)}")
    print(f"eval_dataset size {len(eval_dataset_merge)}")
