...
class CMRLayer(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs: Any
        ...
        x_ffn = self.ffn_fn(*input) # share layer
        # l_aux = {"moe_gate_loss": l_aux}
        ...
        used_budget = (gates * (~input_padding_mask)).sum() # moe使用的token数
        total_budget = (~input_padding_mask).sum() # 总共token数

        l_aux["cmr_gate_loss_num"] = used_budget # loss数量？token数？
        l_aux["cmr_gate_loss_denom"] = total_budget # ？什么denom ，是那个0.8吗
