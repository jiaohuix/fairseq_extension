...
def main() -> None:
    ...
    parser.add_argument("-m", "--model-path", type=str, required=True,
                        help="The pre-trained  model path.(eg: ckpt/model.pt)")
    parser.add_argument("--pt-dict", type=str, required=True, help="The pre-trained model dictionary. (dict.txt)")
    ...
    parser.add_argument("--output", type=str, required=True, help="The trimmed model path.")
    args = parser.parse_args()

    langs = args.langs.split(",")
    pre_dict = load_dict(langs, args.pt_dict, is_ptm=True)
    ...
    for name in ["encoder.embed_tokens.weight", "decoder.embed_tokens.weight", "decoder.output_projection.weight"]:
        ...
        ft_tensor = torch.zeros(
            [len(ft_dict), pre_tensor.shape[1]], dtype=pre_tensor.dtype, layout=pre_tensor.layout,
