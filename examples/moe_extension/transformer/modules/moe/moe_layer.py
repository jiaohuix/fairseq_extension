...
from ..dist import distributed_utils
...
class MOELayer(Base):
    ...
    def forward(
        ...
        # Forward pass of the Positions-wise MoE layer. The underscored letter (e.g., G and
        # E) indicates the dimension along which a tensor will be partitioned.

        d_model = input.shape[2]
        # Pad to expected batch size
        input_shape = list(input.shape) # [128,52,512]
        expected_bsz = (
            getattr(self.args, "batch_size", 0) # [128]
            if self.training
        ...
        return combined_output, {"moe_gate_loss": l_aux} # balance loss
