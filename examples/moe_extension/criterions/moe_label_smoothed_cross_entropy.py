...
from .fairseq_criterion import MoECriterion,MoECriterionConfig
from fairseq.criterions import FairseqCriterion, register_criterion
from ..transformer.modules.moe import MOELayer
from ..logging.meters import GroupedAverageMeter
...
class MoELabelSmoothedCrossEntropyCriterionConfig(MoECriterionConfig):
    ...
    # 应该是balance loss,wt是系数的意思，加权ce balance cmr
    ...
    # lcmr = budget of moe  0.8 is better
...
class MoELabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_loss(self, model, net_output, sample, sample_size, reduce=True):
        ...
        gate_loss = sample_size * gate_loss  # gshard balance loss
        cmr_gate_loss = sample_size * cmr_gate_loss  # cmr loss budget
        loss = (
            ls_loss
    ...
    def get_moe_metadata(self, model):
        ...
        for key in MoELabelSmoothedCrossEntropyCriterion.moe_logging_keys:
            ...
            if key == "median_prefix_count_expert1":
                ...
                # print(encoder_vals,len(encoder_vals))
                # assert len(encoder_vals) > 0,">0"
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "moe_gate_loss", float(moe_loss_sum/sample_size) , sample_size, round=8
        )
        metrics.log_scalar(
