...
class QFormer(nn.Module):
    def __init__(self, n_query=8, nmt_dim=512, bert_dim=768, n_heads=8, drop=0., q_ffn=False):
        super(QFormer, self).__init__()
        self.n_query = n_query
        self.nmt_dim = nmt_dim
        ...
        self.ffn = nn.Linear(nmt_dim, nmt_dim) if q_ffn else None
    ...
    def forward(self, bert_out):
        ...
        q, attn_weights = self.qformer(q, bert_out, bert_out)
        q = q.transpose(0, 1)
...
class BertNMTEncoder(TransformerEncoderBase):
    ...
    def __init__(self, cfg, dictionary, embed_tokens, return_fc=False):
        ...
        # qformer
        self.qformer = QFormer(n_query=cfg.n_query,
                               nmt_dim=cfg.encoder_embed_dim,
                               bert_dim=self.bert_out_dim,
                               n_heads=cfg.encoder_attention_heads,  # 可能得用decoder的
                               drop=cfg.q_drop,
                               q_ffn=cfg.q_ffn)
...
@register_model_architecture('bert_nmt', 'bnmt_iwslt_de_en')
...
#### 原始transformer,加深解码器
@register_model_architecture("transformer", "transformer_iwslt_de_en_dec7")
def transformer_iwslt_de_en7(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 1024)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 4)
    args.encoder_layers = getattr(args, "encoder_layers", 6)
...
@register_model_architecture("transformer", "transformer_iwslt_de_en_dec8")
def transformer_iwslt_de_en8(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 1024)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 4)
    args.encoder_layers = getattr(args, "encoder_layers", 6)
