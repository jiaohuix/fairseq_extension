...
'''
warmup_from_nmt: bool
warmup_nmt_file: ckpt_nmt.pt? 怎么感觉多此一举？直接restore-file不就行了？
'''
...
# def remove_module_params(model, module_names=["bert_encoder"]):
def remove_module_params(model, module_names=[]): # 不擅长bert参数了
    """
    从模型中删除指定模块的指定参数。
...
def get_lora_params(model):
    # https://github.com/huggingface/peft/blob/202f1d7c3d3f8dc468a989f9fb757e319be73d6f/src/peft/peft_model.py#L114
    lora_params= {"cfg": None,
                  "state": None}
    logger.info("save lora config and weight...")
    from peft import get_peft_model_state_dict
    # from peft import get_peft_model_state_dict, set_peft_model_state_dict,    PromptLearningConfig
    # 将lora的参数和config保存到字典
    # if getattr(self.model, "peft_config", None) is not None:
    if safe_hasattr(model,"peft_config"):
        # save the config and change the inference mode to `True`
        peft_config = model.peft_config

        # peft_config.base_model_name_or_path = (
        #     self.model.base_model.bert_encoder.__dict__.get("name_or_path", None)
        #     if isinstance(peft_config, PromptLearningConfig)
        #     else self.model.bert_encoder.base_model.model.__dict__.get("name_or_path", None)
        # )
        inference_mode = peft_config.inference_mode
        peft_config.inference_mode = True
        lora_params["cfg"] = peft_config.__dict__
        peft_config.inference_mode = inference_mode

        # params
        if safe_hasattr(model, "bert_encoder"):
            lora_dict = get_peft_model_state_dict(model.bert_encoder)
            lora_params["state"] = lora_dict

    return lora_params

def load_lora_params(model, lora_params):
    if lora_params is None: return
    # https://github.com/huggingface/peft/blob/202f1d7c3d3f8dc468a989f9fb757e319be73d6f/src/peft/peft_model.py#L172
    from peft import set_peft_model_state_dict, LoraConfig, PeftModel,get_peft_model,LoraModel
    logger.info("load lora config and weight...")
    cfg = lora_params.get("cfg",None)
    state = lora_params.get("state",None)
    if cfg is not None and state is not None:
        logger.info("loaded lora!")
        # 先加载config和权重
        peft_cfg_js = lora_params["cfg"]
        lora_state = lora_params["state"]
        lora_cfg = LoraConfig(**peft_cfg_js)  # 加载配置

        # 配置要修改inference model

        # 加载参数
        if safe_hasattr(model, "bert_encoder"):
            # lora model
            if not isinstance(model.bert_encoder, (LoraModel)):
                model.bert_encoder  = get_peft_model(model.bert_encoder, lora_cfg)
            else:
                model.bert_encoder  = get_peft_model(model.bert_encoder.base_model, lora_cfg)

            set_peft_model_state_dict(model.bert_encoder, lora_state)
    else:
        logger.info("not load lora inload_lora_params............")
...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss, warmup_from_nmt=False):
    ...
    end_of_epoch = epoch_itr.end_of_epoch() if not warmup_from_nmt else True # <--- bert fused
...
def load_checkpoint(cfg: CheckpointConfig, trainer, **passthrough_args):
    ...
    *passthrough_args* will be passed through to
    ...
    if cfg.warmup_from_nmt:
        if os.path.isfile(cfg.warmup_nmt_file):
    ...
    extra_state = trainer.load_checkpoint(
        ...
        warmup_from_nmt=cfg.warmup_from_nmt, # <--- bert fused
...
def load_model_ensemble(
    ...
    num_shards=1,
    state=None,
    bert_ratio=None, encoder_ratio=None
):
...
def load_model_ensemble_and_task(
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            ###############################
            # if bert_ratio is not None:
            #     cfg.bert_ratio = bert_ratio
            #     setattr(cfg.model, 'bert_output_layer', bert_output_layer)
            # if encoder_ratio is not None:
            #     cfg.encoder_ratio = encoder_ratio
            #     setattr(cfg.model, 'bert_output_layer', bert_output_layer)
            #
            # bert_output_layer = getattr(cfg, 'bert_output_layer', -1)
            # setattr(cfg, 'bert_output_layer', bert_output_layer)
            ###############################
            ...
            if "fsdp_metadata" in state and num_shards > 1:
                ...
                #### Load LoRA ###
                # lora_params = state.get("lora", None)
                # if safe_hasattr(model,"use_lora") :
                #     if lora_params is not None:
                #         load_lora_params(model, lora_params=lora_params)
                #         del state["lora"]
                #     else:
                #         print("lora params is None!")
