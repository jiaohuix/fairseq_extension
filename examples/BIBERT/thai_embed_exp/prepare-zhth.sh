echo "bash prepare-ikcest22.sh <ops=8000> "
# params
centric_lang=zh
noncentric_langs=( "th" )
...
bpe_folder=datasets/bpe_${bpe_ops}

git clone https://gitee.com/miugod/nmt_data_tools.git
pip install  subword-nmt zhconv pythainlp
            ...
            do
                if  [ -e $raw_folder/$folder/$prefix.$lang ]; then
                    ...
                    if  [ "$prefix"x == "train"x ]; then
                        ...
                        python nmt_data_tools/my_tools/json2dict.py $tmp_folder/$folder/$prefix.bpe.$lang.json $bpe_folder/$folder/dict.$lang.txt
                    fi
