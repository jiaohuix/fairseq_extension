...
class BertFusedConfig(TransformerConfig):
    ...
    bert_gates: List[int] = field(default_factory=lambda: [1, 1, 1, 1, 1, 1],
                                  metadata={"help": "weather to fuse bert states  each layer."})
...
class BertFusedTransformerModel(TransformerModel):
    def __init__(self, args, encoder, decoder,  bertencoder, berttokenizer):
        ...
        self.args = args # args和cfg有什么区别?到底用哪个
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument('--pooler-type', default="avg", help="[avg/cnn/rand]")
        # dynamic gate
        parser.add_argument('--dynamic-gate', action='store_true',)
        parser.add_argument('--gate-type', default="simple", help="[simple/glu/glunorm]")
    ...
    def build_model(cls, args, task):
        ...
        cfg = BertFusedConfig.from_namespace(args) # ?何用
        ####################################### BERT-FUSED ##############################################
    ...
    def forward(
        ...
        bert_encoder_out = self.bert_encoder(bert_input, attention_mask= ~bert_encoder_padding_mask)[0] #直接获取最后一层输出，没返回n层states
        # hidden_states = self.bert_encoder(src_tokens, attention_mask= ~bert_encoder_padding_mask, output_hidden_states=True)[2]
        # bert_encoder_out = hidden_states[self.bert_output_layer] # default take last hidden state of bert
        ...
        encoder_out = self.encoder(
            ...
            bert_encoder_out=bert_encoder_out # <--bert fused
        )
        # 3. forward decoder
...
class BertFusedEncoder(TransformerEncoderBase):
    def __init__(self, cfg, dictionary, embed_tokens, return_fc=False):
        ...
        self.layers.extend([
            ...
            self.build_encoder_layer(cfg,bert_gates[i])
            for i in range(cfg.encoder.layers)
        ])
        ####################################### BERT-FUSED ##############################################

    def build_encoder_layer(self, cfg, bert_gate=1):
        layer = BertFusedEncoderLayer(
            cfg, bert_gate=bert_gate
        )
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            lr = layer(
                ...
                bert_encoder_out = bert_encoder_out['bert_encoder_out'],
                bert_encoder_padding_mask = bert_encoder_out['bert_encoder_padding_mask'])
            ####################################### BERT-FUSED ##############################################
    ...
    def reorder_encoder_out(self, encoder_out, bert_out, new_order):
        ...
        if bert_out['bert_encoder_out'] is not None:
            bert_out['bert_encoder_out'] = \
                bert_out['bert_encoder_out'].index_select(1, new_order)
    ...
    def forward_torchscript(self, net_input: Dict[str, Tensor], bert_encoder_out=None):
        ...
        if torch.jit.is_scripting():
            ...
            return self.forward_non_torchscript(net_input, bert_encoder_out)

    @torch.jit.unused
    def forward_non_torchscript(self, net_input: Dict[str, Tensor], bert_encoder_out):
        encoder_input = {
            k: v for k, v in net_input.items()
            if k != 'prev_output_tokens' and k != 'bert_input'
        }
        encoder_input['bert_encoder_out'] = bert_encoder_out
...
class BertFusedDecoder(TransformerDecoderBase):
    ...
    def build_decoder_layer(self, cfg, no_encoder_attn=False, bert_gate=1):
        layer =  BertFusedDecoderLayer(cfg, no_encoder_attn, bert_gate = bert_gate)
        checkpoint = cfg.checkpoint_activations
        if checkpoint:
            offload_to_cpu = cfg.offload_activations
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
    ...
    def extract_features(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, _ = layer(
                ...
                padding_mask,
                bert_encoder_out['bert_encoder_out'],  # <--- bert fused
                bert_encoder_out['bert_encoder_padding_mask'],  # <--- bert fused
                incremental_state,
...
def bert_fuse_base_arch(args):
    ...
    args.gate_type = getattr(args, "gate_type", "simple")
