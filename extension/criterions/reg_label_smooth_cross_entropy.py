...
'''
name: RDrop
source code: https://github.com/dropreg/R-Drop/blob/main/fairseq_src/examples/translation_rdrop/translation_rdrop_src/loss/reg_label_smooth_cross_entropy.py
...
@dataclass
class RdropCriterionConfig(LabelSmoothedCrossEntropyCriterionConfig):
    reg_alpha: float = field(
        default=5.,
        metadata={"help": "Rdrop coef, default=5 for translation."},
    )
...
@register_criterion(
    "reg_label_smoothed_cross_entropy", dataclass=RdropCriterionConfig
)
class RegLabelSmoothedCrossEntropyCriterion(FairseqCriterion):

    def __init__(self, task, sentence_avg, label_smoothing, reg_alpha):
        super().__init__(task)
        self.sentence_avg = sentence_avg
        self.eps = label_smoothing
        self.reg_alpha = reg_alpha  # 构建criterion时会从cfg里找初始化参数然后criterion(**init_args)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # }
        loss, sample_size, logging_output = self.forward_reg(model, sample, reduce=True)
