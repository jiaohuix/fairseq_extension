...
python scripts/prep_data.py -i $dataset -o $data_outdir -v dict/codes.bpe.32000.txt # merge bpe ops
...
do
    ...
    do
        awk '{print "'"$src_lang_id"' " $0}' $train_data_dir/$prefix.$src_lang  > $train_data_dir/$prefix.src
        awk '{print "'"$tgt_lang_id"' " $0}' $train_data_dir/$prefix.$tgt_lang  > $train_data_dir/$prefix.tgt
    done
    ...
    for prefix in train valid test
    do
        awk '{print "'"$tgt_lang_id"' " $0}' $train_data_dir/$prefix.$tgt_lang  > $train_data_dir/$prefix.src
        awk '{print "'"$src_lang_id"' " $0}' $train_data_dir/$prefix.$src_lang  > $train_data_dir/$prefix.tgt
    done
...
bash scripts/bin.sh  $train_multi_dir  $data_bin_multi_dir  src tgt 1 dict/bpe_vocab.txt
...
do
    ...
    bash scripts/bin.sh  $train_dual_dir  $data_bin_dual_dir  src tgt 1 dict/bpe_vocab.txt
    
    # 单向
    echo "lang_pair: $lang_pair uni"
    train_data_dir=$data_outdir/$wandb_proj/$lang_pair
    data_bin_dir=data-bin/${wandb_proj}/${lang_pair} 
    bash scripts/bin.sh  $train_data_dir  $data_bin_dir  src tgt 1  dict/bpe_vocab.txt
