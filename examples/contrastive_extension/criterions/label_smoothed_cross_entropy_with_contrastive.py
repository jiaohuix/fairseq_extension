...
class LabelSmoothedCrossEntropyCriterionWithContrastive(
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # contrastive_lambda是句子级别的损失，/nsentences后是一个token损失，需要再*ntokens
    ...
    def get_contrastive_loss(self, encoder_out1, encoder_out2, sample1, sample2):
        ...
        def _sentence_embedding(encoder_out, sample):
            encoder_output = encoder_out[0].transpose(0, 1) # error2: list can't transpose # [T B C]-> [B T C]
        ...
        loss = -nn.LogSoftmax(0)(torch.div(anchor_dot_contrast, self.temperature)).diag().sum() # calculated on the sentencelevel
