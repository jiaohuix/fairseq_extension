...
from .dist import dist_utils
from fairseq import utils
from fairseq.modules import LayerNorm, gelu
...
from .fused_bias_relu_squared import fused_bias_relu_squared
from .linear import Linear
from .moe import CMRLayer, MOELayer, Top1Gate, Top2Gate
...
class TransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def forward(
        ...
        if not run_moe:# bottom不是moe当然没有损失
            x, fc_result = _ffn(
                x,
                self.fc1,
                self.activation_fn,
